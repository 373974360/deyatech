package com.deyatech.workflow.custom;
...
public class MyBpmnJsonConverter extends BpmnJsonConverter {
    ...
    private static final Logger LOGGER = LoggerFactory.getLogger(MyBpmnJsonConverter.class);

    private static Map<String, Class<? extends BaseBpmnJsonConverter>> convertersToBpmnMap = new HashMap<>();

    private static Map<Class<? extends BaseElement>, Class<? extends BaseBpmnJsonConverter>> convertersToJsonMap = new HashMap<>();
    ...
    public ObjectNode covertToJson(BpmnModel model) {
        ...
        maxX += 50;
        maxY += 50;
        maxX = maxX < 1485 ? 1485 : maxX;
        maxY = maxY < 700 ? 700 : maxY;
        ...
        if (CollectionUtil.isNotEmpty(model.getPools())) {
        ...
        if (StringUtils.isNotBlank(mainProcess.getId())) {
            propertiesNode.put(PROPERTY_PROCESS_ID, mainProcess.getId());
        }
        if (StringUtils.isNotBlank(mainProcess.getName())) {
            propertiesNode.put(PROPERTY_NAME, mainProcess.getName());
        }
        if (StringUtils.isNotBlank(mainProcess.getDocumentation())) {
        ...
        if (StringUtils.isNotBlank(model.getTargetNamespace())) {
        ...
        if (CollectionUtil.isNotEmpty(mainProcess.getDataObjects())) {
        ...
        if (CollectionUtil.isNotEmpty(model.getPools()) && poolHasDI) {
            GraphicInfo graphicInfo = null;
            for (Pool pool : model.getPools()) {
                graphicInfo = model.getGraphicInfo(pool.getId());
                if (graphicInfo == null) {
                ...
                if (StringUtils.isNotBlank(pool.getName())) {
                ...
                if (process != null) {
                    ...
                    for (Lane lane : process.getLanes()) {
                        ...
                        laneNode.put("outgoing", objectMapper.createObjectNode());
                    ...
                    for (FlowElement flowElement : process.getFlowElements()) {
                        ...
                        GraphicInfo flowLaneGraphicInfo = null;
                        ...
                        for (Lane lane : process.getLanes()) {
                            if (lane.getFlowReferences().contains(lookForElement.getId())) {
                                flowLaneGraphicInfo = model.getGraphicInfo(lane.getId());
                                if (flowLaneGraphicInfo != null) {
                                    laneForElement = lane;
                                }
                                break;
                            }
                        ...
                        if (flowElement instanceof SequenceFlow || laneForElement != null) {
                            processFlowElement(flowElement, process, model, laneMap.get(laneForElement.getId()),
                                    flowLaneGraphicInfo.getX(), flowLaneGraphicInfo.getY());
                        }
                    }
    ...
    public BpmnModel convertToBpmnModel(JsonNode modelNode) {
            ...
            if (STENCIL_POOL.equals(stencilId)) {
                ...
                for (JsonNode laneNode : laneArrayNode) {
                    ...
                    if (STENCIL_LANE.equals(laneStencilId)) {
                        ...
                        if (CollectionUtil.isNotEmpty(lane.getFlowReferences())) {
        ...
        if (!nonEmptyPoolFound) {
            ...
            if (processExecutableNode != null && StringUtils.isNotBlank(processExecutableNode.asText())) {
            ...
            for (JsonNode shapeNode : shapesArrayNode) {
                if (STENCIL_SEQUENCE_FLOW.equalsIgnoreCase(BpmnJsonConverterUtil.getStencilId(shapeNode))
                    ...
                    if (StringUtils.isNotBlank(sourceRef)) {
                        ...
                        SequenceFlowJsonConverter flowJsonConverter = new SequenceFlowJsonConverter();
                        if (lane != null) {
                            flowJsonConverter.convertToBpmnModel(shapeNode, modelNode, this,
                                    lane, shapeMap, bpmnModel);
                        } else {
                            flowJsonConverter.convertToBpmnModel(shapeNode, modelNode, this,
        ...
        for (Process process : bpmnModel.getProcesses()) {
            ...
            if (subShapesMap.size() > 0) {
                List<String> removeSubFlowList = new ArrayList<>();
                ...
                for (FlowElement flowElement : process.findFlowElementsOfType(SequenceFlow.class)) {
                    ...
                    if (subShapesMap.containsKey(sequenceFlow.getSourceRef())) {
                        ...
                        if (subProcess.getFlowElement(sequenceFlow.getId()) == null) {
                            ...
                            removeSubFlowList.add(sequenceFlow.getId());
                        }
                    }
                }

                for (String flowId : removeSubFlowList) {
                ...
                for (ExtensionElement orderElement : orderList) {
                    ...
                    if (StringUtils.isNotBlank(flowValue)) {
                        if (allFlowMap.containsKey(flowValue)) {
                            ...
                            flowWithContainer.getFlowContainer().removeFlowElement(
    ...
    public void processJsonElements(JsonNode shapesArrayNode, JsonNode modelNode, BaseElement parentElement,
        ...
        for (JsonNode shapeNode : shapesArrayNode) {
            ...
            try {
                ...
                converterInstance.convertToBpmnModel(shapeNode, modelNode, this,
                        parentElement, shapeMap, bpmnModel);
            } catch (Exception e) {
    ...
    protected void processFlowElement(FlowElement flowElement, FlowElementsContainer container, BpmnModel model,
        ...
        if (converter != null) {
            try {
                converter.newInstance().convertToJson(flowElement, this, model, container,
    ...
    protected void processMessageFlows(BpmnModel model, ArrayNode shapesArrayNode) {
        for (MessageFlow messageFlow : model.getMessageFlows().values()) {
            ...
            jsonConverter.convertToJson(messageFlow, this, model, null, shapesArrayNode,
                    0.0, 0.0);
        }
    }
    ...
    private void readEdgeDI(Map<String, JsonNode> edgeMap, Map<String, List<JsonNode>> sourceAndTargetMap,
        ...
        for (String edgeId : edgeMap.keySet()) {
            ...
            if (!"".equals(edgeNode.get("properties").get("name"))) {
                bpmnModel.addLabelGraphicInfo(edgeId, createGraphicInfo(graphicInfoList.get(0).getX(),
            ...
            if (flowElement instanceof Event) {
                ...
                if (CollectionUtil.isNotEmpty(event.getEventDefinitions())) {
                    ...
                    if (eventDef instanceof SignalEventDefinition) {
                        ...
                        if (StringUtils.isNotBlank(signalEventDef.getSignalRef())) {
                        ...
                        if (StringUtils.isNotBlank(messageEventDef.getMessageRef())) {
