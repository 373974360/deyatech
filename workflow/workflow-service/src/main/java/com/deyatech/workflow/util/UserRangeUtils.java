...
public class UserRangeUtils {
    ...
    public static AbstractUserRange getUserRange(String key) {
        ...
        String[] keys = StringUtils.split(key, Constants.WORKFLOW_RANG_KEY_SPLIT);
        if (null != keys && keys.length == 3) {
            FlowForm form = WorkFlowConfig.getEntityForm(keys[0] + Constants.WORKFLOW_RANG_KEY_SPLIT + keys[1]);
