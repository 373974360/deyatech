...
docker run -d --restart=always -m=512m --name consul --network deyatech --privileged=true -v /deya/data/consul/consul_kv.json:/consul/kv/consul_kv.json:ro  -e 'CONSUL_LOCAL_CONFIG={"skip_leave_on_interrupt": true}' consul agent -server -bind=127.0.0.1  -bootstrap-expect=1 -ui -client 0.0.0.0
#启动redis，并且设置自定义密码
